...
class MultiModalModel(nn.Module):
    def __init__(self, dropout_rate=0.7):
        ...
        self.text_branch = nn.Sequential(
            nn.Conv1d(in_channels=100, out_channels=256, kernel_size=5, padding=2),
            nn.LayerNorm([256, 63]),
            nn.LeakyReLU(negative_slope=0.01),
            nn.Conv1d(in_channels=256, out_channels=100, kernel_size=5, padding=2),  # 修改此行
            nn.LayerNorm([100, 63]),
            nn.LeakyReLU(negative_slope=0.01),
        )
        ...
        self.audio_branch = nn.Sequential(
            nn.Conv1d(in_channels=73, out_channels=256, kernel_size=5, padding=2),
            nn.LayerNorm([256, 63]),
            nn.LeakyReLU(negative_slope=0.01),
            nn.Conv1d(in_channels=256, out_channels=100, kernel_size=5, padding=2),
            nn.LayerNorm([100, 63]),
            nn.LeakyReLU(negative_slope=0.01),
        )
        # 视频分支
    ...
    def forward(self, text, audio, video, mask):
        ...
        video_embeddings = video_embeddings.transpose(1, 2)
